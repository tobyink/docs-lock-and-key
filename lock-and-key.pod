=head1 Keeping Your Valuables Under Lock and Key
...
invalidate the cached lookup hash held in C<< _lookup >>, making the
C<< lookup_name >> method no longer work reliably.
...
it doesn't do anything to I<< prevent >> them adding months (or removing them!)
the old way.

=head2 Internals::SvREADONLY to the rescue

C<< Internals::SvREADONLY >> is a Perl internal function for marking a
...
By adding a one line C<< BUILD >> method (the C<< BUILD >> method is
automatically called by the constructor in classes based on Moose, Mouse,
Moo, Class::Tiny, etc) we can lock down the C<months> array:

  sub BUILD ( $self, $arg ) {
    Internals::SvREADONLY( $self->months->@*, 1 );
  }

Our C<push_month> method will need a few changes to be able to alter the
read-only array:
...
One thing to note is that C<< Internals::SvREADONLY >> is extremely shallow.
It will prevent items being added to or removed from the C<months> array, but
...
=head2 Sub::Trigger::Lock

A while ago I wrote a module that packages up this behaviour for
L<Moose>, L<Mouse>, L<Moo>, and sufficiently-compatible frameworks.

First of all, let's rewrite our original class using L<Moo>.
...
As before, it is possible to directly push to the C<months> array:

  push $list->months->@*, 'Extrember';     # add an extra month

L<Sub::Trigger::Lock> will lock down the attribute:

  use Sub::Trigger::Lock -all;
  has months => ( is => 'ro', isa => ArrayRef, trigger => Lock );

And our C<push_month> method becomes:
...
What is this C<< $guard >> variable? It is an object which will re-lock the
C<< $self->months >> array after it has gone out of scope.

While L<Sub::Trigger::Lock> doesn't fully recurse into locked data structures,
...
L<Mite> also makes locking attributes reasonably easy, using
C<< locked => true >> in the attribute definition. The C<push_month>
method can also be included declaratively via Mite's support for
C<< handles_via => 'Array' >>. The only additional step is an
C<< after push_month >> method modifier to clear the C<_lookup> hashref.
...
=head2 Alternative approaches

An alternative approach to locking attributes is cloning them. The basic idea
is whenever somebody requests C<< $list->months >>, instead of returning a
...
L<MooseX::Extended> offers a C<clone> feature to make this approach easy.
L<Mite> also supports C<clone>. One drawback is that this can be an expensive
...
C<< Internals::SvREADONLY >>, but modules like L<Sub::Trigger::Lock> exist
...
L<https://github.com/tobyink/docs-lock-and-key>.
